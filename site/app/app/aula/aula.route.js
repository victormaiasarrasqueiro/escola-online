...
(function() {
    ...
    function getStates() {
        return [
			...
			{
				state: 'app.aulaplaylist',
				config: {
					url: '/aula/playlist',
					title: 'Carregando Youtube playlist',
					views: {
						'content@app': {
							templateUrl: 'app/aula/playlist.aula.html',
							controller: 'PlaylistAulaController',
							controllerAs: 'vm'
						}
					},
					data: {
				...
				config: {
					...
					resolve:{
						...
						aula: function(CRUDService,$stateParams){
							...
							return CRUDService.aula().obter($stateParams.id).then(function(data){ return data; });
